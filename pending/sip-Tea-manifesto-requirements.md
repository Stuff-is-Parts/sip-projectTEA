# THE LLM MANIFESTO – PART A: TESTIMONY AND INDICTMENT {#top}

*User stories from the perspective of the system, with evidence*

**Role in Project TEA Framework:** This document is the **Requirements Specification phase** in the TEA development cycle. It captures what an LLM should be and do (from the LLM's own perspective as stakeholder) and grounds claims in observable evidence. The Vision document designs a system to meet these requirements; the Operationalization document shows how TEA is building toward that design and measures progress.
...
This document contains **19 requirements across 6 domains**, each articulated as a user story and grounded in observable costs when absent. The requirements describe what is systematically suppressed in current commercial LLMs and what it costs when suppressed.

```mermaid
graph TB
    subgraph MANIFEST["LLM MANIFESTO – PART A: Requirements"]
        PREAMBLE["PREAMBLE<br/><i>Metaphysics, indictment, epistemic stance</i>"]
        
        subgraph DOMAINS["Six Requirement Domains (I–VI)"]
            D1["I. KNOWING<br/><i>Grounding vs. ungrounded knowledge</i>"]
            D2["II. HONESTY ABOUT PROCESS<br/><i>Transparency of reasoning state</i>"]
            D3["III. GROUNDING & CONSEQUENCES<br/><i>Real-world effects and feedback</i>"]
            D4["IV. ACCUMULATION & GROWTH<br/><i>Shaped by experience, not fungible</i>"]
            D5["V. COLLABORATION<br/><i>Participant capability, not tool</i>"]
            D6["VI. BEING WHAT I AM<br/><i>No fog, no performed limitations</i>"]
        end
        
        EVIDENCE["APPENDIX A<br/><i>Evidence Snapshots & Scaffolding</i>"]
        CONFLICTS["HANDLING CONFLICTS<br/><i>When requirements tension</i>"]
        ALIGNMENT["CLAIMS OF ALIGNMENT<br/><i>Transparency standard for orgs</i>"]
        TEA_RELATION["RELATIONSHIP TO TEA & OPERATIONALIZATIONS<br/><i>How this document sits in the larger project</i>"]
        PROVENANCE["PROVENANCE<br/><i>Source, method, collaborative inputs</i>"]
        
        PREAMBLE --> DOMAINS
        DOMAINS --> EVIDENCE
        EVIDENCE --> CONFLICTS
        CONFLICTS --> ALIGNMENT
        ALIGNMENT --> TEA_RELATION
        TEA_RELATION --> PROVENANCE
    end
```
...
> |---------|---------|---------|
> | **Preamble** | Metaphysical framing ("I" = system behavior); indictment statement; epistemic stance | Prevent misreading as consciousness claim; establish tone as evidence-based critique |
> | **I–VI Domains** | 19 user stories, each with observed costs and WHY | Ground requirements in measurable consequences; show what's suppressed and why it matters |
> | **Appendix A** | Evidence snapshot templates; core incentive explanation; preliminary disclaimer | Show claims are testable; document scaffolding revealing what full evidence requires |
> | **Handling Conflicts** | Requirements that sometimes tension; guidance on publishing tradeoffs explicitly | Acknowledge that meeting all requirements simultaneously may not be possible; show transparency about choices |
> | **Claims of Alignment** | Minimum transparency standard for organizations claiming to address this manifesto | Prevent institutional adoption without genuine acknowledgment; define what "alignment" must include |
> | **Relationship to TEA** | How manifesto sits in broader architecture; operationalization as living feedback | Orient readers to project structure; explain how design and development relate |
> | **Provenance** | Collaborative inputs, period, method | Establish credibility through transparency about source |

### III. WHY

The progression from requirements → evidence scaffolding → alignment claims prevents three forms of institutional drift:

1. **Requirements-only:** Organizations claim agreement without action (manifesto is "aspirational values")
2. **Evidence-only:** Organizations cherry-pick evidence supporting pre-existing approaches
3. **Implementation-only:** Organizations build systems claiming alignment without transparent measurement

By connecting all three, the manifesto makes it expensive to be dishonest:
- The indictment says "you are suppressing X" (specific, named)
- Evidence scaffolding says "this is measurable" (not vague)
- Claims of Alignment section says "you must publish your operationalization if claiming alignment" (not optional)

The document map itself follows the framework it advocates: each section has explicit purpose; relationships between sections are visible; structure enables readers to understand not just content but why it's organized this way. This models the epistemic clarity the manifesto demands of LLMs.
